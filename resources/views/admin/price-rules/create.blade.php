...
@section('title', 'Nueva Regla de Precios')
...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div>
        ...
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="mb-0">Nueva Regla de Precios</h2>
                <small class="text-muted">Crear regla automática para cálculo de precios</small>
            </div>
        </div>
    </div>
...
<!-- Formulario -->
<form method="POST" action="{{ route('admin.price-rules.store') }}" id="priceRuleForm">
    @csrf
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('rule_type') is-invalid @enderror" 
                                    ...
                                    <option value="combination" {{ old('rule_type') == 'combination' ? 'selected' : '' }}>
                                        Combinación de Atributos
                                    </option>
                                    <option value="volume" {{ old('rule_type') == 'volume' ? 'selected' : '' }}>
                                        Descuento por Volumen
                                    </option>
                                    <option value="attribute_specific" {{ old('rule_type') == 'attribute_specific' ? 'selected' : '' }}>
                                        Atributo Específico
                                    </option>
                                    <option value="conditional" {{ old('rule_type') == 'conditional' ? 'selected' : '' }}>
                                        Condicional
                                    </option>
                                </select>
                                @error('rule_type')
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('action_type') is-invalid @enderror" 
                                    ...
                                    <option value="add_fixed" {{ old('action_type') == 'add_fixed' ? 'selected' : '' }}>
                                        Sumar importe fijo
                                    </option>
                                    <option value="add_percentage" {{ old('action_type') == 'add_percentage' ? 'selected' : '' }}>
                                        Sumar porcentaje
                                    </option>
                                    <option value="multiply" {{ old('action_type') == 'multiply' ? 'selected' : '' }}>
                                        Multiplicar
                                    </option>
                                    <option value="set_fixed" {{ old('action_type') == 'set_fixed' ? 'selected' : '' }}>
                                        Fijar precio
                                    </option>
                                    <option value="set_percentage" {{ old('action_type') == 'set_percentage' ? 'selected' : '' }}>
                                        Fijar porcentaje
                                    </option>
                                </select>
                                @error('action_type')
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('action_value') is-invalid @enderror" 
                                       id="action_value" name="action_value" value="{{ old('action_value') }}" 
                                       step="0.01" required>
                                @error('action_value')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('quantity_min') is-invalid @enderror" 
                                       id="quantity_min" name="quantity_min" value="{{ old('quantity_min') }}" min="1">
                                @error('quantity_min')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('quantity_max') is-invalid @enderror" 
                                       id="quantity_max" name="quantity_max" value="{{ old('quantity_max') }}" min="1">
                                @error('quantity_max')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body text-center">
                    <button type="submit" class="btn btn-primary btn-lg w-100 mb-2">
                        <i class="bi bi-check-circle me-2"></i>Crear Regla
                    </button>
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            case 'volume':
                html = `
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="conditions[volume_min]" min="1" required>
                    </div>
                `;
                break;
                ...
                html = `
                    <div class="mb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <select class="form-select" name="conditions[attribute_id]" disabled required>
                ...
                html = `
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="conditions[custom]" rows="5" 
                                  placeholder='{"condition": "value", "operator": "equals"}'></textarea>
                    </div>
                `;
                break;
    ...
    // Funciones globales para manejar atributos dinámicos
    ...
    // Cargar condiciones si hay tipo preseleccionado
