        ...
        <div class="card">
            ...
            <div class="card-body">
                @php
                    ...
                    $productPrintColors = old('print_colors', $product->print_colors ?? []);
                @endphp
                                    ...
                                    <div class="col-md-3 mb-3 print-color-item" data-print-color-id="{{ $color->id }}">
                                        <div class="form-check">
                                            <input class="form-check-input print-color-checkbox" type="checkbox" 
                                                   ...
                                                   data-hex="{{ $color->hex_code }}"
                                                   {{ in_array($color->name, $productPrintColors) ? 'checked' : '' }}>
                                            <label class="form-check-label d-flex align-items-center" 
                                            ...
                                            <tr class="pricing-row">
                                                <td>
                                                    <input type="number" class="form-control" name="pricing[{{ $index }}][quantity_from]" 
                                                           value="{{ $pricing['quantity_from'] ?? $pricing->quantity_from ?? '' }}" placeholder="1" min="1" required>
                                                </td>
                                                <td>
                                                    <input type="number" class="form-control" name="pricing[{{ $index }}][quantity_to]" 
                                                           value="{{ $pricing['quantity_to'] ?? $pricing->quantity_to ?? '' }}" placeholder="100" min="1" required>
                                                </td>
                                                <td>
                                                    <input type="number" class="form-control pricing-total" name="pricing[{{ $index }}][price]" 
                                                           value="{{ $pricing['price'] ?? $pricing->price ?? '' }}" placeholder="0.00" step="0.01" min="0" required>
                                                </td>
                                                <td>
                                                    <input type="number" class="form-control pricing-unit" name="pricing[{{ $index }}][unit_price]" 
                                                           value="{{ $pricing['unit_price'] ?? $pricing->unit_price ?? '' }}" placeholder="0.00" step="0.01" min="0" required>
                                                </td>
                                                <td>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    @foreach($customFields as $field)
                                        @php
                                            $fieldValue = old('custom_fields.' . $field->id, $product->custom_fields[$field->id] ?? '');
                                        @endphp
                                        <div class="mb-3">
                                            ...
                                            @switch($field->field_type)
                                                ...
                                                @case('textarea')
                                                    <textarea class="form-control" 
                                                              ...
                                                              rows="3"
                                                              placeholder="{{ $field->placeholder }}"
                                                              {{ $field->required ? 'required' : '' }}>{{ $fieldValue }}</textarea>
                                                    @break
...
document.addEventListener('DOMContentLoaded', function() {
    // Usar el mismo JavaScript del create.blade.php
    // Todo el código es idéntico, solo cambian los valores iniciales
    
    let sizeIndex = {{ $product->sizes ? count($product->sizes) : 1 }};
    let pricingIndex = {{ $product->pricing->count() }};

    // Copiar todo el JavaScript del archivo create.blade.php aquí
    // El código es exactamente el mismo, funcionará para editar también
    ...
    if (categorySelect && subcategorySelect) {
        categorySelect.addEventListener('change', function() {
            ...
            // Si la subcategoría actual no pertenece a la nueva categoría, resetear
            const currentSubcategory = subcategorySelect.value;
            if (currentSubcategory) {
                const selectedOption = subcategorySelect.querySelector(`option[value="${currentSubcategory}"]`);
